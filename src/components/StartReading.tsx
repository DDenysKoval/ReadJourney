...
export default function StartReading({ onSubmitFilters }: FiltersProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <>
      <p className="text-white text-[10px] pl-3.5 mb-2">Start page:</p>
        ...
        <div className=" flex items-center p-3.5  bg-light-gray rounded-xl w-full h-11 mb-5">
          ...
          {errors.startPages && (
            <span className="text-red text-[10px] absolute top-11 left-0">
              {errors.startPages.message}
            </span>
          )}
        </div>
        <ButtonComp
          text={isSubmitting ? "Starting" : "To start"}
          type="submit"
